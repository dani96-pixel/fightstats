// event.js — Vista de evento con fixture grid estilo FootyStats

const FLAG_LABELS = {
  long_layoff:                'Layoff largo (>365d)',
  very_long_layoff:           'Layoff muy largo (>540d)',
  low_ufc_experience:         'Poca exp. UFC',
  high_finish_dependency:     'Alta dep. finalizaciones',
  moderate_finish_dependency: 'Dep. finalizaciones moderada',
  severe_recent_decline:      'Declive severo reciente',
  moderate_recent_decline:    'Declive moderado reciente',
};

// Estado del panel de detalle abierto
let openFightId = null;

async function renderEvent(eventId) {
  const container = document.getElementById('event-content');
  container.innerHTML = '<div class="loading">Cargando análisis</div>';
  ...
  try {
    ...
    container.innerHTML = errorState('No se encontró el evento.');
    return;
  }
  ...
  try {
    ...
    container.innerHTML = errorState('No se pudo cargar el evento.');
    return;
  }

  const ev = eventData.event;

  // Agrupar peleas por sección
  const sections = groupFights(eventData.fights);

  container.innerHTML = `
    <button class="back-btn" id="back-to-events">Volver a eventos</button>

    <div class="event-detail-header">
      <div class="event-label">Análisis Nivarax</div>
      <h2>${ev.name}</h2>
      <div class="event-meta">
        ...
        <span class="status-badge ${ev.status}">${statusLabel(ev.status)}</span>
      </div>
    </div>

    ${eventData.fights.length === 0
        ? '<div class="loading" style="padding:60px 0;color:var(--text-muted);">Análisis próximamente</div>'
        : renderSections(sections, ev.status)
    }

    <!-- Panel de detalle — se actualiza al hacer click en una fixture card -->
  ...
  return sections.map(s => `
    <div class="fixture-section">
      <div class="fixture-section-header">
        ...
        <span class="fixture-section-count">${s.fights.length} pelea${s.fights.length > 1 ? 's' : ''}</span>
      </div>
...
// ─────────────────────────────────────────────
// FIXTURE CARD (la card compacta del grid)
// ─────────────────────────────────────────────
function renderFixtureCard(fight, eventStatus) {
  ...
  if (isCompleted && result) {
    ...
    resultInfo = `<div class="fixture-pending">Pendiente</div>`;
  }

  // Barra de probabilidad mini
  const probBar = `
  ...
  return `
    <div class="fixture-card ${cardMod}" data-fight-id="${fight.id}"
      ...
      <div class="fixture-expand-hint" id="hint-${fight.id}">Ver análisis ▾</div>
    </div>
  `;
}
...
function toggleFightDetail(fightId, fight, eventStatus) {
  ...
  if (openFightId === fightId) {
    ...
    if (hint) hint.textContent = 'Ver análisis ▾';
    return;
  }

  // Cerrar el anterior si había otro abierto
  if (openFightId) {
    ...
    if (prevHint) prevHint.textContent = 'Ver análisis ▾';
  }
  ...
  // Mover el panel justo debajo de la card
...
function renderDetailPanel(fight, eventStatus) {
  ...
  if (isCompleted && result) {
    ...
    resultBlock = `
      <div class="dp-result ${correct ? 'dp-result--correct' : 'dp-result--incorrect'}">
        <span>${correct ? '✓ Nivarax acertó' : '✗ Nivarax falló'}</span>
  ...
  // Stats
  const statsDefs = [
    { label: 'Golpes/min',    key: 'sig_strikes_landed_per_min',   pct: false },
    { label: 'Precisión',     key: 'sig_strike_accuracy',           pct: true },
    { label: 'Recibidos/min', key: 'sig_strikes_absorbed_per_min',  pct: false },
    { label: 'Defensa',       key: 'sig_strike_defense',            pct: true },
    { label: 'Ac. derribo',   key: 'takedown_accuracy',             pct: true },
    { label: 'Def. derribo',  key: 'takedown_defense',              pct: true },
  ];
  ...
  const riskCols = [
    ...
    const chips = [
      ...f.risk.flags.map(fl => `<span class="dp-flag dp-flag--risk">${FLAG_LABELS[fl] || fl}</span>`),
      ...f.deg.flags.map(fl => `<span class="dp-flag dp-flag--deg">${FLAG_LABELS[fl] || fl}</span>`),
    ];
    const flagsHTML = chips.length
      ? chips.join('')
      : `<span class="dp-flag dp-flag--ok">Sin alertas</span>`;
      ...
      <div class="dp-risk-col">
        ...
        <div class="dp-risk-scores">
          <div class="dp-risk-item">
            <div class="dp-risk-label">Incertidumbre</div>
          ...
          <div class="dp-risk-item">
            <div class="dp-risk-label">Degradación</div>
    ...
    <div class="detail-panel-inner">
      ...
      <div class="dp-adv">
        ...
        <div class="dp-adv-labels">
          ...
          <span class="dp-adv-neutral">neutro</span>
      ...
      <div class="dp-section">
        <div class="dp-section-title">Estadísticas comparadas</div>
      ...
      <!-- Risk -->
      <div class="dp-section">
        <div class="dp-section-title">Riesgo e Incertidumbre</div>
        <div class="dp-risk-grid">${riskCols}</div>
      </div>

      <!-- Key factors -->
      <div class="dp-section">
        <div class="dp-section-title">Factores clave</div>
...
function statusLabel(s) {
  return { completed: 'Completado', upcoming: 'Próximo' }[s] || s;
}
